...
class DecBlock(nn.Module):
    ...
    def sample_reg(self, x, zx, acts, a, b, t=None, mode='map'):
    ...
    def forward_reg(self, xs, zx, activations, a, b, get_latents=False, t=None, mode='map'): # TODO: mode in 'sample' , 'map'
        x, acts = self.get_inputs(xs, activations)
        if self.mixin is not None:
            x = x + F.interpolate(xs[self.mixin][:, :x.shape[1], ...], scale_factor=self.base // self.mixin)
        z, x, kl, ll = self.sample_reg(x, zx, acts, a, b, t=t, mode=mode)
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        stats = dict(kl=kl, ll=ll)
...
class Decoder(HModule):
    ...
    def forward_reg(self, activations, latents_x, a, b, get_latents=False, t=None, nmax=None, mode='map'):
        ...
        for idx, (block, zl) in enumerate(zip(self.dec_blocks, latents_x)):
            try:
                ...
                temp = t
            if idx < nmax:
                xs, block_stats = block.forward_reg(xs, zl, activations, a, b, get_latents=get_latents, t=temp, mode=mode)
                stats.append(block_stats)
            else:
...
class VAE(HModule):
    ...
    def forward_with_latent_reg(self, x, a, b, get_latents=False, presample_z=False, t=None, nmax=None, quantize=False, mode='sample'):#TODO: mode
        """_summary_
        ...
        px_z, stats = self.decoder.forward_reg(activations, latents_x, a, b, get_latents=get_latents, t=t, nmax=nmax, mode=mode)
